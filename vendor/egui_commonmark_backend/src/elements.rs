...
pub fn code_block<'t>(
    ...
    // PATCH(Clarity TagFlow): draw the app's copy/copied SVG icons instead of
    // the 🗐 / ✔ text glyphs, tinted to the theme's text colour.
    let copy_button = ui
        .put(
            egui::Rect {
                min: position,
            ...
            egui::Button::image(
                egui::Image::new(if copied_icon {
                    egui::include_image!("./assets/copied.svg")
                } else {
                    egui::include_image!("./assets/copy.svg")
                })
