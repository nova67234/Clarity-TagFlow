...
impl CommonMarkViewerInternal {
    ...
    fn event_text(&mut self, text: CowStr, ui: &mut Ui) {
        ...
        if let Some(image) = &mut self.image {
            ...
            // PATCH(Clarity TagFlow): render color emoji as inline Twemoji
            // SVG images — egui's font rasterizer is monochrome-only, so
            // emoji would otherwise show as pale outlines or tofu. Non-emoji
            // runs keep their original styling via `to_richtext`.
            let mut run = String::new();
            for g in unicode_segmentation::UnicodeSegmentation::graphemes(text.as_ref(), true) {
                match twemoji_svg(g) {
                    Some(asset) => {
                        if !run.is_empty() {
                            ui.label(self.text_style.to_richtext(ui, &run));
                            run.clear();
                        }
                        // Sized slightly above the font so it reads at the
                        // same visual weight as the surrounding text.
                        let size = ui
                            .style()
                            .text_styles
                            .get(&TextStyle::Body)
                            ...
                            * 1.15;
                        let mut uri = String::from("twemoji-");
                        for (i, c) in g.chars().enumerate() {
                            if i > 0 {
                                uri.push('-');
                            }
                            uri.push_str(&format!("{:x}", c as u32));
                        }
                        uri.push_str(".svg");
                        ui.add(
...
/// PATCH(Clarity TagFlow): the Twemoji asset for an emoji grapheme. U+FE0F
/// (the emoji-presentation variation selector, e.g. in 🌧️ / ❤️) is
/// inconsistent between real-world text and the asset table's keys, so an
/// exact miss retries with the selector stripped and then re-appended.
fn twemoji_svg(grapheme: &str) -> Option<&'static twemoji_assets::svg::SvgTwemojiAsset> {
    ...
    SvgTwemojiAsset::from_emoji(&with_vs)
}

/// PATCH(Clarity TagFlow): true when any grapheme in `text` has a Twemoji
/// asset — the cheap pre-scan that keeps the common no-emoji path on the
/// original single-label rendering.
fn text_has_emoji(text: &str) -> bool {
    unicode_segmentation::UnicodeSegmentation::graphemes(text, true)
        .any(|g| twemoji_svg(g).is_some())
}
